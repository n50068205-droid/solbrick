...
function App() {
  ...
  const onboardSteps = [
    ...
    {icon:'💰', title:'Инвестируй от $25', desc:'Покупай доли в реальных проектах через смарт-контракт'},
    {icon:'📈', title:'Получай доход', desc:'Средний ROI 13.5% годовых. Прозрачно on-chain.'},
  ...
  if (showOnboarding) return (
    <div style={{fontFamily:"'Segoe UI',sans-serif", background:S.bg, minHeight:'100vh', color:S.text, display:'flex', alignItems:'center', justifyContent:'center', padding:20}}>
      <style>{`@keyframes fi{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <div style={{maxWidth:400, width:'100%', animation:'fi 0.5s ease'}}>
        <div style={{textAlign:'center', marginBottom:32}}>
          <div style={{width:64, height:64, background:S.green, borderRadius:18, display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, margin:'0 auto 12px'}}>🏗</div>
        ...
        <div style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:16, padding:28, textAlign:'center', marginBottom:16}}>
          <div style={{fontSize:48, marginBottom:14}}>{onboardSteps[onboardStep].icon}</div>
        ...
        <div style={{display:'flex', justifyContent:'center', gap:6, marginBottom:16}}>
          {onboardSteps.map((_,i) => <div key={i} style={{width:i===onboardStep?20:6, height:6, background:i===onboardStep?S.green:S.border, borderRadius:3, transition:'all 0.3s'}} />)}
        </div>
        <button onClick={() => onboardStep < onboardSteps.length-1 ? setOnboardStep(s=>s+1) : setShowOnboarding(false)}
          style={{width:'100%', background:S.green, color:S.bg3, border:'none', padding:'14px', borderRadius:10, cursor:'pointer', fontSize:15, fontWeight:700, marginBottom:8}}>
    ...
    <div style={{fontFamily:"'Segoe UI',sans-serif", background:S.bg, minHeight:'100vh', color:S.text, width:'100%', overflowX:'hidden'}}>
      <style>{`
        *{box-sizing:border-box;}
        ...
        .card:hover{background:#1e2330 !important;}
      ...
      {/* Header */}
      <header style={{background:S.bg2, borderBottom:`1px solid ${S.border}`, padding:`0 ${isMobile?'12px':'32px'}`, display:'flex', alignItems:'center', justifyContent:'space-between', height:52, position:'sticky', top:0, zIndex:100}}>
        <div style={{display:'flex', alignItems:'center', gap:8}}>
          <div style={{width:30, height:30, background:S.green, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16}}>🏗</div>
          <span style={{fontSize:16, fontWeight:700}}>SolBrick</span>
          <span style={{background:S.bg3, color:S.green, fontSize:9, padding:'2px 6px', borderRadius:4, border:`1px solid ${S.green}30`, fontWeight:700, letterSpacing:1}}>DEVNET</span>
        </div>
        ...
        <div style={{display:'flex', alignItems:'center', gap:8}}>
          ...
          {wallet ? (
            ...
            <button onClick={connectWallet} className="btn"
              style={{background:S.green, color:S.bg3, border:'none', padding:`7px ${isMobile?'10px':'16px'}`, borderRadius:8, fontSize:isMobile?12:13, fontWeight:700, whiteSpace:'nowrap'}}>
              {isMobile ? '🔗' : '🔗 Подключить'}
            </button>
          )}
        </div>
      ...
      {/* Photo Modal */}
      {photoModal && (
        <div style={{position:'fixed', inset:0, zIndex:2000, background:'rgba(0,0,0,0.92)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:isMobile?12:20}}
          ...
          <div style={{width:'100%', maxWidth:800}} onClick={e => e.stopPropagation()}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10}}>
              ...
              <button onClick={() => setPhotoModal(null)} style={{background:'transparent', border:`1px solid ${S.border}`, color:S.text2, padding:'4px 10px', borderRadius:6, cursor:'pointer', fontSize:12}}>✕</button>
            </div>
            <div style={{borderRadius:10, overflow:'hidden', marginBottom:10, position:'relative'}}>
              <img src={photoModal.project.photos[photoModal.idx]} alt="" style={{width:'100%', height:isMobile?220:380, objectFit:'cover', display:'block'}} />
              {photoModal.idx > 0 && <button onClick={() => setPhotoModal(p => p?{...p,idx:p.idx-1}:null)} style={{position:'absolute', left:8, top:'50%', transform:'translateY(-50%)', background:'rgba(0,0,0,0.6)', border:'none', color:'white', width:32, height:32, borderRadius:'50%', cursor:'pointer', fontSize:16}}>‹</button>}
              {photoModal.idx < photoModal.project.photos.length-1 && <button onClick={() => setPhotoModal(p => p?{...p,idx:p.idx+1}:null)} style={{position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', background:'rgba(0,0,0,0.6)', border:'none', color:'white', width:32, height:32, borderRadius:'50%', cursor:'pointer', fontSize:16}}>›</button>}
            </div>
            <div style={{display:'flex', gap:8, justifyContent:'center', marginBottom:12}}>
              {photoModal.project.photos.map((ph: string, i: number) => (
                <img key={i} src={ph} alt="" onClick={() => setPhotoModal(p => p?{...p,idx:i}:null)}
                  style={{width:60, height:44, objectFit:'cover', borderRadius:6, border:`2px solid ${i===photoModal.idx?S.green:S.border}`, cursor:'pointer', opacity:i===photoModal.idx?1:0.6}} />
              ))}
            </div>
            <button onClick={(e) => {setPhotoModal(null); openModal(photoModal.project, 1, e);}} className="btn"
              style={{width:'100%', background:S.green, color:S.bg3, border:'none', padding:'12px', borderRadius:8, fontSize:14, fontWeight:700}}>
      ...
      {modal && (
        <div style={{position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', padding:16}}
          ...
          <div style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:16, padding:24, width:'100%', maxWidth:380, animation:'modalIn 0.25s ease'}}
            onClick={e => e.stopPropagation()}>
            <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:16, paddingBottom:12, borderBottom:`1px solid ${S.border}`}}>
              <span style={{fontSize:28}}>{modal.project.image}</span>
              <div>
            ...
            <div style={{background:'#0d2d1e', border:`1px solid ${S.green}30`, borderRadius:8, padding:'8px 12px', margin:'12px 0', fontSize:12, color:S.green}}>
              ⚡ Запись в Solana блокчейн
            </div>
            <button onClick={confirmBuy} disabled={txPending} className="btn"
              style={{width:'100%', background:txPending?S.border:S.green, color:txPending?S.text2:S.bg3, border:'none', padding:'12px', borderRadius:8, fontSize:14, fontWeight:700, marginBottom:8, display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>
      ...
      {/* Message */}
      {message && (
        <div style={{position:'fixed', top:60, left:'50%', transform:'translateX(-50%)', zIndex:500, background:message.includes('✅')?'#0d2d1e':'#2d1515', border:`1px solid ${message.includes('✅')?S.green+'40':S.red+'40'}`, borderRadius:8, padding:'10px 20px', fontSize:13, color:message.includes('✅')?S.green:S.red, whiteSpace:'nowrap', animation:'fadeUp 0.3s ease'}}>
          {message}
        </div>
      )}
      ...
      <main style={{maxWidth:1280, margin:'0 auto', padding:isMobile?'12px 12px 80px':'24px 32px'}}>
        ...
        {activePage==='home' && (
          <div style={{animation:'fadeUp 0.3s ease'}}>
            {/* Banner */}
            <div style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12, padding:isMobile?'16px':'20px 28px', marginBottom:16}}>
              ...
              <h1 style={{fontSize:isMobile?20:24, fontWeight:700, margin:'0 0 6px'}}>Инвестируй в недвижимость от $25</h1>
              <p style={{color:S.text2, fontSize:13, margin:'0 0 14px'}}>Токенизированные проекты Казахстана на блокчейне</p>
              <div style={{display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10, marginBottom:14}}>
                {[{l:'Проектов',v:counter.projects,c:S.green},{l:'Объём',v:`$${counter.volume}K`,c:S.blue},{l:'Инвесторов',v:`${counter.investors.toLocaleString()}+`,c:'#A78BFA'},{l:'Avg ROI',v:`${counter.roi}%`,c:'#F59E0B'}].map((s,i) => (
                  <div key={i} style={{background:S.bg3, border:`1px solid ${S.border}`, borderRadius:8, padding:'10px 14px', textAlign:'center'}}>
                    <div style={{fontSize:18, fontWeight:700, color:s.c, fontFamily:'monospace'}}>{s.v}</div>
                    <div style={{fontSize:10, color:S.text3, textTransform:'uppercase', letterSpacing:0.5, marginTop:2}}>{s.l}</div>
                  </div>
                ))}
              </div>
              {!isMobile && (
                <button onClick={() => setActivePage('calculator')} className="btn"
                  style={{background:'transparent', color:S.green, border:`1px solid ${S.green}40`, padding:'8px 16px', borderRadius:8, fontSize:13, fontWeight:600}}>
                  Рассчитать доход →
                </button>
              )}
            </div>
            ...
            <div style={{display:'flex', gap:6, marginBottom:14, flexWrap:'wrap', overflowX:'auto', paddingBottom:4}}>
              {tags.map(tag => (
                <button key={tag} onClick={() => setFilterTag(tag)} className="btn"
                  style={{background:filterTag===tag?S.green:S.bg2, color:filterTag===tag?S.bg3:S.text2, border:`1px solid ${filterTag===tag?S.green:S.border}`, padding:'6px 12px', borderRadius:6, fontSize:12, fontWeight:filterTag===tag?600:400, whiteSpace:'nowrap', flexShrink:0}}>
                  {tag}
                </button>
              ))}
            </div>
              ...
              {filtered.map((asset, idx) => {
                ...
                return (
                  <div key={asset.id} className={`card ${isExp?'expanded':''}`}
                    ...
                    style={{background:S.bg2, border:`1px solid ${isExp?S.green:S.border}`, borderRadius:12, overflow:'hidden', animation:loaded?`fadeUp ${0.2+idx*0.05}s ease`:'none'}}>
                    <div style={{position:'relative', height:isMobile?130:150, overflow:'hidden'}}>
                      ...
                      <div style={{position:'absolute', top:8, left:8, display:'flex', gap:5}}>
                        ...
                        {isHot && <span style={{background:S.red, color:'white', fontSize:10, padding:'2px 7px', borderRadius:4, fontWeight:700}}>HOT</span>}
                      </div>
                      <button onClick={(e) => {e.stopPropagation(); setPhotoModal({project:asset, idx:0});}}
                        style={{position:'absolute', top:8, right:8, background:'rgba(0,0,0,0.6)', border:`1px solid ${S.border}`, color:'white', padding:'3px 8px', borderRadius:6, cursor:'pointer', fontSize:11}}>
                        📷
                      </button>
                      <div style={{position:'absolute', bottom:8, left:10, right:10, display:'flex', justifyContent:'space-between', alignItems:'flex-end'}}>
                        ...
                        <div style={{textAlign:'right'}}>
                          <div style={{fontSize:16, fontWeight:700, color:S.green, fontFamily:'monospace'}}>{asset.roi}</div>
                    ...
                    <div style={{padding:'10px 12px'}}>
                      <div style={{marginBottom:8}}>
                        ...
                        <div style={{background:S.bg3, borderRadius:2, height:3}}>
                          <div style={{background:isHot?S.red:asset.color, borderRadius:2, height:3, width:`${pct}%`}} />
                        </div>
                      </div>

                      <div style={{overflow:'hidden', maxHeight:isExp?'500px':'0', opacity:isExp?1:0, transition:'all 0.35s ease'}}>
                        <div style={{display:'flex', gap:5, marginBottom:8}}>
                          {asset.photos.map((ph: string, i: number) => (
                            <img key={i} src={ph} alt="" onClick={(e) => {e.stopPropagation(); setPhotoModal({project:asset, idx:i});}}
                              style={{flex:1, height:48, objectFit:'cover', borderRadius:5, border:`1px solid ${S.border}`, cursor:'pointer'}} />
                          ))}
                        </div>
                        ...
                        <button onClick={(e) => {e.stopPropagation(); openModal(asset, 1, e);}} className="btn"
                          style={{width:'100%', background:asset.color, color:S.bg3, border:'none', padding:'10px', borderRadius:8, fontSize:13, fontWeight:700}}>
                      ...
                      {!isExp ? (
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                          ...
                          <span style={{color:asset.color, fontSize:11}}>▼ открыть</span>
                        </div>
        ...
        {activePage==='portfolio' && (
          <div style={{animation:'fadeUp 0.3s ease'}}>
            ...
            {!wallet ? (
              <div style={{textAlign:'center', padding:40, background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12}}>
                <div style={{fontSize:36, marginBottom:10}}>🔗</div>
                <p style={{color:S.text2, fontSize:13, marginBottom:16}}>Подключите кошелёк</p>
                <button onClick={connectWallet} className="btn" style={{background:S.green, color:S.bg3, border:'none', padding:'10px 20px', borderRadius:8, fontSize:14, fontWeight:700}}>Подключить Phantom</button>
              </div>
            ) : (
              <>
                <div style={{display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12, marginBottom:16}}>
                  {[{l:'Портфель',v:`$${totalPortfolio}`,c:S.green},{l:'Свободно',v:`$${usdBalance.toLocaleString()}`,c:S.blue},{l:'SOL',v:`◎ ${solBalance.toFixed(3)}`,c:'#A78BFA'},{l:'Проектов',v:Object.keys(portfolio).length,c:'#F59E0B'}].map((s,i) => (
                    <div key={i} style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:10, padding:'14px 16px'}}>
                      <div style={{fontSize:10, color:S.text3, textTransform:'uppercase', letterSpacing:0.5, marginBottom:4}}>{s.l}</div>
                ...
                {Object.keys(portfolio).length === 0 ? (
                  <div style={{textAlign:'center', padding:32, background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12}}>
                    ...
                    <button onClick={() => setActivePage('home')} className="btn" style={{background:S.green, color:S.bg3, border:'none', padding:'8px 16px', borderRadius:8, fontSize:13, fontWeight:700, marginTop:10}}>К проектам</button>
                  </div>
                ) : (
                  <div style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12, overflow:'hidden'}}>
                    {Object.entries(portfolio).map(([id,shares],i) => {
                      ...
                      return (
                        <div key={id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 16px', borderTop:i>0?`1px solid ${S.border2}`:'none'}}>
                          <div style={{display:'flex', alignItems:'center', gap:12}}>
                            <img src={p.photos[0]} alt="" style={{width:44, height:44, borderRadius:8, objectFit:'cover'}} />
                            <div>
        ...
        {activePage==='calculator' && (
          <div style={{animation:'fadeUp 0.3s ease'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:16}}>
              ...
              <div style={{background:'#0d2d1e', border:`1px solid ${S.green}30`, borderRadius:12, padding:20}}>
                <div style={{fontSize:12, color:S.green, fontWeight:600, marginBottom:12, textTransform:'uppercase', letterSpacing:0.5}}>Результат</div>
                <div style={{fontSize:32, fontWeight:700, color:S.green, fontFamily:'monospace', marginBottom:16}}>${calcResult.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g,',')}</div>
                  ...
                  return (
                    <div key={i} style={{display:'flex', alignItems:'center', gap:8, marginBottom:6}}>
                      ...
                      <div style={{flex:1, background:S.bg3, borderRadius:2, height:4}}>
                        <div style={{background:S.green, borderRadius:2, height:4, width:`${Math.min((yv/calcResult)*100,100)}%`}} />
                      </div>
                      <span style={{fontSize:11, color:S.green, fontFamily:'monospace', width:70, textAlign:'right'}}>${yv.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g,',')}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {activePage==='transactions' && (
          <div style={{animation:'fadeUp 0.3s ease'}}>
            ...
            <p style={{color:S.text2, fontSize:13, margin:'0 0 20px'}}>Все on-chain операции</p>
            {transactions.length === 0 ? (
              <div style={{textAlign:'center', padding:40, background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12}}>
                <div style={{fontSize:36, marginBottom:10}}>📭</div>
                <p style={{color:S.text2, fontSize:13}}>Транзакций пока нет</p>
              </div>
            ) : (
              <div style={{background:S.bg2, border:`1px solid ${S.border}`, borderRadius:12, overflow:'hidden'}}>
                {transactions.map((tx,i) => (
                  <div key={tx.id} style={{padding:'14px 16px', borderTop:i>0?`1px solid ${S.border2}`:'none'}}>
                    ...
                    <span style={{background:'#0d2d1e', color:S.green, fontSize:10, padding:'2px 8px', borderRadius:4, fontWeight:600}}>BUY · Confirmed</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {!isMobile && (
          <div style={{marginTop:40, paddingTop:16, borderTop:`1px solid ${S.border2}`, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
            <span style={{fontSize:11, color:S.text3}}>SolBrick © 2026 · National Solana Hackathon</span>
      ...
      {/* Mobile Bottom Nav */}
      {isMobile && (
        <div style={{position:'fixed', bottom:0, left:0, right:0, zIndex:200, background:S.bg2, borderTop:`1px solid ${S.border}`, display:'flex', justifyContent:'space-around', padding:'8px 0 16px', backdropFilter:'blur(20px)'}}>
          {[['home','🏠','Проекты'],['portfolio','💼','Портфель'],['calculator','🧮','Расчёт'],['transactions','📋','История']].map(([page,icon,label]) => (
            <button key={page} onClick={() => setActivePage(page as Page)}
              style={{background:'transparent', border:'none', cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:2, padding:'4px 8px', color:activePage===page?S.green:S.text2}}>
